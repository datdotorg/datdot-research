...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
  plan // publish, make CON_TRACTSs, make hosting_setups
  => scheduleActionPriority // plan, CON_TRACTSs, hosting_setups
  => PendingQueue
  => tryExecute
  => if success, schedule followup

  // const CON_TRACTS = { id, feedID, chunks, history: [37]... }
  // const entry =  { id: 37, type: 'job', data: { providers } }
  // CON_TRACTS.history.push({ type: 'job', data: { providers: { hosters: [2,7,6], encoder: [1,9,3], attestors: [12] } } })
  // CON_TRACTS.history.push({ type: 'cancel', data: { block: 77 } })
  // CON_TRACTS.history.push({ type: 'job', data: { providers: { hosters: [2] } } })
  ...
  // 5. if plan     : EXECUTE "plan ACTION": split into sets of ranges, make CON_TRACTSs, add providers journal entry, emit job event
  // 6. if fail     : (no providers) => add CON_TRACTS ACTION to priority queue (`ratio` based)
  // ...
  // 7. chain       : take highest priority items and execute (e.g. CON_TRACTS ACTION)
  // 8. if CON_TRACTS : EXECUTE "CON_TRACTS ACTION": find providers, add journal entry, emit job event + (schedule FollowUp)
...
// ------------------------------------------------------------------------------
    ...
    function plan_update (id, update) {
      ...
      // e.g. PLAN = 0-100 chunks => 10 hosting_setups
      // 1: 0-9
      // 2: 10-19
      // ...
      // => 1. schedule plan
      // => 2. on time: add to prio queue
      // => 3. on prio: make hosting_setups + start hosting
      ...
      // CASE: priority queued
        ...
        // CASE: hosting_setups (=priority queue)
        if (!dropped) queue.drop(new_plan.hosting_Setups)
      }
